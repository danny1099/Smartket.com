...
Public Class commision_periods_show
    Private period As Periods = Periods.Instance
    Private criteria_search As String = "p.row_visible=3"
...
#Region "constructor"
    ...
    Private Sub module_load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cmb_period_status.Datasources(period.commision_periods_status, "status_name")
    End Sub
#End Region
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = period.commision_periods_show(criteria_search)
    ...
    Private Sub status_changed(sender As Object, e As EventArgs) Handles cmb_period_status.EditValueChanged
        If cmb_period_status.EditValue IsNot Nothing Then
            reports_show("p.row_visible=" & cmb_period_status.EditValue)
        End If
    End Sub
...
#Region "options"
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New commision_periods_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        period.commision_periods_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub finalized_option(sender As Object, e As EventArgs) Handles btn_object_finally.Click
        If dgv_object_view.RowCount > 0 Then
            If message_text("Está seguro que desea finalizar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        period.commision_periods_closed(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub searched_option(sender As Object, e As EventArgs) Handles btn_object_find.Click
        Using new_ As New commision_periods_search
            new_.ShowDialog(start_home)

            'comprueba que el query de busqueda tenga datos
            If new_.condition_criteria <> "" Then
                reports_show(new_.condition_criteria)
            End If
        End Using
    End Sub
