...
Public Class commision_searched_show
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = commision.commision_payments_show(criteria_search)
...
#Region "helpers"
    Private Sub objects_condition()
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de liquidación", "p.payment_type", GetType(Integer), New RepositoryListed(commision.commision_payments_types("row_visible=1 and t.Id in (1,2,3)"), "commision_type"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Estado de liquidación", "p.status_code", GetType(Integer), New RepositoryListed(commision.commision_payments_status("row_visible=1 and s.Id not in (6)"), "status_name"), FilterColumnClauseClass.Lookup))
    End Sub
...
#Region "options"
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.RowCount > 0 Then
            If message_text("Está seguro que desea modificar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                ...
                'Comprueba que la lista de filas seleccionadas sea mayor a 0
                If rows_selected.Count > 0 Then show_option(New commision_settleds_edited(rows_selected)) Else message_text("No se detectó filas seleccionadas", MessageBoxButtons.OK)
            End If
        End If
    End Sub
    ...
    Private Sub erased_option(sender As Object, e As EventArgs) Handles btn_object_erase.Click
        If dgv_object_view.RowCount > 0 Then
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        commision.commision_payments_erase(dgv_object_view.GetRowCellValue(i, "Id"), dgv_object_view.GetRowCellValue(i, "wholesales_code"), False)
                    End If
                Next
    ...
    Private Sub discount_option(sender As Object, e As EventArgs) Handles btn_object_discount.Click
        If dgv_object_view.RowCount > 0 Then
            If message_text("Está seguro que desea crear un descuento de comisión a los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
