...
Public Class commision_rules_edited
...
#Region "behaviors"
    ...
    Private Sub object_search()
        Dim table As DataTable = rules.commision_rules_search("r.Id=" & record_affected)

        If table.Rows.Count = 1 Then
            With table.Rows(0)
                cmb_rules_type.EditValue = .Item("rule_type").ToString
                txt_rules_name.EditValue = .Item("rule_name").ToString
                cmb_rules_segment.EditValue = cmb_rules_segment.Properties.GetKeyValueByDisplayText(.Item("segment_name").ToString)
                cmb_rules_operator.EditValue = cmb_rules_operator.Properties.GetKeyValueByDisplayText(.Item("operator_name").ToString)
                cmb_rules_payment.EditValue = cmb_rules_payment.Properties.GetKeyValueByDisplayText(.Item("commision_type").ToString)
                cmb_rules_function.EditValue = .Item("function_name").ToString
                txt_rules_criteria.FilterString = .Item("condition_object").ToString
    ...
    Private Sub criteria_condition(segment_code As Integer)
        ...
        If segment_code = 1 Then
            ...
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Portabilidad", "s.mark_ported", GetType(Integer), New RepositoryListed(parameter.settings_search_yesno2, "yes_no"), FilterColumnClauseClass.Lookup))
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Migración", "s.mark_migration", GetType(Integer), New RepositoryListed(parameter.settings_search_yesno2, "yes_no"), FilterColumnClauseClass.Lookup))
    ...
    Private Sub saved_option(sender As Object, e As EventArgs) Handles btn_object_accept.Click
        If xvp_validate.Validate = True Then
            With rules.sql_command
                ...
                .CommandText = "commision_scheme_rules_edited"
