...
Public Class relationship_budget_showed
    ...
    Private criteria_search As String = "b.validity_month=" & Month(Now) & " and b.validity_year=" & Year(Now) & " and b.row_visible = 1 and b.agency_code In (" & sessions.agency_permit & ")"
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = budget.relationship_budgets_show(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Mes").AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center
                .Columns("Año").AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Function total_values() As String
        ...
        For i As Integer = 0 To dgv_object_view.DataRowCount
            value_total += CDbl(Replace(Replace(Replace(dgv_object_view.GetRowCellValue(i, "Presupuesto"), ".", ""), "$", ""), "- ", "-"))
        Next
...
#Region "helpers"
    Private Sub objects_condition()
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de registro", "convert(date,u.object_date)", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Total Presupuesto", "convert(decimal(18,2),b.budget_total)", GetType(Integer), New RepositoryText, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de presupuesto", "b.operand_type", GetType(String), New RepositoryListed(scales.commision_scales_types, "scale_type"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Agencia", "a.agency_code", GetType(Integer), New RepositoryChecked(agency.settings_agencys_search("row_visible=1 and a.Id in (" & sessions.agency_permit & ")"), "agency_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Mes", "b.validity_month", GetType(Integer), New RepositoryChecked(parameter.settings_search_months, "month_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Año", "b.validity_year", GetType(Integer), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
    End Sub
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New relationship_budget_created)
    End Sub
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                show_option(New relationship_budget_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        Else
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        budget.relationship_budgets_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
