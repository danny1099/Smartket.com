...
Public Class comercial_attention_show
    Private attention As Attentions = Attentions.Instance
    Private notes_ As Notes = Notes.Instance
    Private criteria_search As String = "month(t.created_date)=month('" & Now.ToString("yyyy-MM-dd") & "') and year(t.created_date)=year('" & Now.ToString("yyyy-MM-dd") & "') and t.row_visible=1 and t.agency_code in (" & sessions.agency_permit & ") and t." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim ticket_source As DataTable = attention.commercial_attention_show(criteria_search)
        ...
        If ticket_source.Rows.Count > 0 Then
            'define el origen de datos al grid control
            dgv_object_grid.DataSource = ticket_source
            ...
            With dgv_object_view
                ...
                'change alignment of quotation
                .Columns("Cotizaciones").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .Columns("Ventas").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .Columns("Efectividad").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles lbl_object_today.Click, lbl_object_month.Click, lbl_object_lastday.Click, lbl_object_open.Click
        reports_show(sender.Tag.ToString & " and t.row_visible=1 and t.agency_code in (" & sessions.agency_permit & ") and t." & sessions.query_permit)
    End Sub

    Private Sub selected_resumen(sender As Object, e As EventArgs) Handles dgv_object_grid.DoubleClick
        If dgv_object_view.IsRowSelected(dgv_object_view.FocusedRowHandle) = True Then
            show_option(New comercial_attention_details(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
        End If
    End Sub
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New comercial_attention_create)
    End Sub

    Private Sub finalize_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        ...
        If dgv_object_view.RowCount > 0 Then
            If message_text("Está seguro que desea transferir el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                ...
                If rows_selected.Count > 0 Then
                    Using new_ As New comercial_attention_transfer(rows_selected)
    ...
    Private Sub resumen_option(sender As Object, e As EventArgs) Handles btn_object_resume.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
    ...
    Private Sub noted_option(sender As Object, e As EventArgs) Handles btn_object_notes.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using new_ As New model_object_notes("Relationship.Customers.Tickets", dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"), "Gestiones")
                new_.ShowDialog(start_home)
            End Using
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
