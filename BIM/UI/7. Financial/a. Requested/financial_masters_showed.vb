...
Public Class financial_masters_showed
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        ...
        source_object = financial.financial_request_showed(criteria_search)
        ...
        If source_object.Rows.Count > 0 Then
            With source_object
                object_label_slopes.Text = String.Format("Pendientes: ({0})", .Select("Estado='Pendiente'").Count)
                object_label_isolates.Text = String.Format("En Verificación: ({0})", .Select("Estado='En Verificación'").Count)
                object_label_approved.Text = String.Format("Aprobados: ({0})", .Select("Estado='Aprobado'").Count)
                object_label_refuse.Text = String.Format("Rechazados: ({0})", .Select("Estado='Rechazado'").Count)
    ...
    Public Sub reports_show(table_source As DataTable)
        ...
        If table_source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("% Interes").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .Columns("Número de cuotas").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles object_label_slopes.Click, object_label_approved.Click, object_label_search.Click, object_label_isolates.Click, object_label_refuse.Click
        If source_object.Rows.Count > 0 Then
    ...
    Private Sub selected_rows(sender As Object, e As EventArgs) Handles dgv_object_grid.DoubleClick
        With dgv_object_view
            If .IsRowSelected(.FocusedRowHandle) = True Then
                show_option(New financial_masters_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        End With
    End Sub
    ...
    Private Function total_values() As String
        ...
        For i As Integer = 0 To dgv_object_view.DataRowCount
            value_total += CDbl(Replace(Replace(Replace(dgv_object_view.GetRowCellValue(i, "Valor a financiar"), ".", ""), "$", ""), "- ", "-"))
        Next
...
#Region "options"
            ...
            If message_text("Está seguro que desea anular los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        financial.financial_request_erased(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub noted_option(sender As Object, e As EventArgs) Handles btn_object_notes.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using new_ As New model_object_notes("Entities.Financial.Request", dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"), "Financiera")
    ...
    Private Sub proof_option(sender As Object, e As EventArgs) Handles btn_object_proof.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Estado").ToString = "Aprobado" Then
                If message_text("Está seguro que desea generar el plan de pagos para el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                    Using new_ As New financial_masters_payment(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                        new_.ShowDialog(start_home)
                    End Using
                End If
            Else
                message_text("La solicitud debe estar aprobada para generar el plan de pagos", MessageBoxButtons.OK)
            End If
        Else
                message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
        End If
    ...
    Private Sub signs_option(sender As Object, e As EventArgs) Handles btn_object_sings.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Estado").ToString = "Aprobado" Then
                If message_text("Está seguro que desea generar los formatos de firma para el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                    Using new_ As New financial_masters_pagare(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                        new_.ShowDialog(start_home)
                    End Using
                End If
            Else
                message_text("La solicitud debe estar aprobada para generar los documentos de firma", MessageBoxButtons.OK)
            End If
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
        End If
    ...
    Private Sub documents_option(sender As Object, e As EventArgs) Handles btn_object_document.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea consultar los documentos adjuntos al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New financial_masters_document(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
