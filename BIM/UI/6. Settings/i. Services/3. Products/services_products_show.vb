...
Public Class services_products_show
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = product.services_products_show(criteria_search)
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New services_products_create)
    End Sub
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                show_option(New services_products_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        Else
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        product.services_products_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub properties_option(sender As Object, e As EventArgs) Handles btn_object_properties.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea agregar propiedades al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New services_products_properties(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
