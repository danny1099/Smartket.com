...
Public Class reporting_comission_resumen
    ...
    Private periods As Periods = Periods.Instance
...
#Region "behaviors"
    Private Sub objects_condition()
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de liquidación", "p.payment_type", GetType(Integer), New RepositoryChecked(commision.commision_payments_types("row_visible=1"), "commision_type"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Usuario Creador", "p.user_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1 and p.agency_code in (" & sessions.agency_permit & ")"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de regla", "p.rule_code", GetType(Integer), New RepositoryChecked(rules.commision_rules_listed("r.row_visible=1"), "Nombre de la regla"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Periodo de pago", "p.period_code", GetType(Integer), New RepositoryChecked(periods.commision_periods_search("p.row_visible=1"), "period_name"), FilterColumnClauseClass.Lookup))
    End Sub
    ...
    Private Sub search_option(sender As Object, e As EventArgs) Handles btn_object_find.Click
        ...
        Else
            If criteria_condition() <> "" Then
                ...
                Dim source As DataTable = reporting.reporting_wholesales_comission(criteria_condition)
                ...
                If source.Rows.Count > 0 Then
                    With crv_object_report
                        ...
                        .LocalReport.ReportEmbeddedResource = "BIM.reporting_comission_resumen.rdlc"
                        .RefreshReport()
                    End With
                End If
            Else
