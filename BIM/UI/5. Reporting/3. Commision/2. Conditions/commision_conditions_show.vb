...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = condition.conditions_schemes_show(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Nombre de esquema").Visible = False
                .Columns("Segmento").Visible = False
                .Columns("Nombre de esquema").GroupIndex = 0
    ...
    Private Sub scheme_changed(sender As Object, e As EventArgs) Handles cmb_condition_scheme.EditValueChanged
        If cmb_condition_scheme.EditValue IsNot Nothing Then
            reports_show(criteria_search & " and c.scheme_code=" & cmb_condition_scheme.EditValue)
        End If
    End Sub
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New commision_conditions_create)
    End Sub
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                show_option(New commision_conditions_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        Else
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        condition.conditions_schemes_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub rules_option(sender As Object, e As EventArgs) Handles btn_object_rules.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Metodo de validación") <> "Liquidación por venta" Then
                If message_text("Está seguro que desea asociar reglas de conteo al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                    show_option(New conditions_counts_show(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Condición")))
                End If
            Else
                message_text("La condición del esquema no admite reglas de conteo", MessageBoxButtons.OK)
            End If
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
        End If
