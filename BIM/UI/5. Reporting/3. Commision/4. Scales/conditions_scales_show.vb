...
Public Class conditions_scales_show
    Private scales_ As Scales = Scales.Instance
    Private condition As Conditions = Conditions.Instance
    Private criteria_search As String = "s.row_visible=1"
...
#Region "behaviors"
    ...
    Private Sub object_listed()
        cmb_scales_condition.Datasources(condition.conditions_schemes_listed("c.row_visible=1"), "condition_name")
    End Sub

    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = scales_.commision_scales_show(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Condición").Visible = False
                .Columns("Esquema de comisión").GroupIndex = 0
                .Columns("Condición").GroupIndex = 1
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub condition_change(sender As Object, e As EventArgs) Handles cmb_scales_condition.EditValueChanged
        If cmb_scales_condition.EditValue IsNot Nothing Then
            reports_show(criteria_search & " and s.condition_code=" & cmb_scales_condition.EditValue)
        End If
    End Sub
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        If cmb_scales_condition.EditValue IsNot Nothing Then
            Using new_ As New conditions_scales_create(cmb_scales_condition.EditValue)
                new_.ShowDialog(start_home)
            End Using

            'update listed of scales
            If cmb_scales_condition.EditValue IsNot Nothing Then reports_show(criteria_search & " and s.condition_code=" & cmb_scales_condition.EditValue) Else reports_show(criteria_search)
        Else
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using edit_ As New conditions_scales_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        scales_.commision_scales_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
