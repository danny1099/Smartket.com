...
Public Class conditions_counts_show
    Private condition As Conditions = Conditions.Instance
    Private rule As Rules = Rules.Instance
    Private criteria_search As String = "r.row_visible=1"
...
#Region "behaviors"
    Private Sub object_listed()
        cmb_rules_condition.Datasources(condition.conditions_schemes_listed("c.row_visible=1"), "condition_name")
        cmb_rules_condition.EditValue = cmb_rules_condition.Properties.GetKeyValueByDisplayText(record_affected)
    End Sub

    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = rule.commision_rules_listed(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("condition_object").Visible = False
                .Columns("segment_code").Visible = False
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub condition_change(sender As Object, e As EventArgs) Handles cmb_rules_condition.EditValueChanged
        If cmb_rules_condition.EditValue IsNot Nothing Then
            reports_show("r.Id in (select rule_code from [Commission.Schemes.Counts] where condition_code=" & cmb_rules_condition.EditValue & " and row_visible=1) and r.segment_code=" & cmb_rules_condition.GetColumnValue("segment_code"))
        End If
    End Sub
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        If cmb_rules_condition.EditValue IsNot Nothing Then
            Using new_ As New conditions_counts_create(cmb_rules_condition.EditValue, cmb_rules_condition.GetColumnValue("segment_code"))
                new_.ShowDialog(start_home)
            End Using
        Else
            message_text("Debe seleccionar una condición de esquema valida", MessageBoxButtons.OK)
        End If
    End Sub

    Private Sub search_option(sender As Object, e As EventArgs) Handles btn_object_view.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using view_ As New commision_rules_view(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "condition_object"), dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "segment_code"))
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        condition.commision_count_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
                ...
                If message_text("Los registros seleccionados fueron anulados exitosamente", MessageBoxButtons.OK) = DialogResult.OK Then
                    'update listed of scales
                    If cmb_rules_condition.EditValue IsNot Nothing Then reports_show("r.Id in (select rule_code from [Commission.Schemes.Counts] where condition_code=" & cmb_rules_condition.EditValue & " and row_visible=1) and r.segment_code=" & cmb_rules_condition.GetColumnValue("segment_code"))
                End If
            End If
        End If
    End Sub
