...
Public Class reports_collections_resumen
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = reporting.reporting_collections_resumen(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("collection_values").Visible = False
                .BestFitColumns(True)
                .SelectRow(0)
...
#Region "helpers"
    Private Sub objects_condition()
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de recaudo", "c.date_collection", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de registro", "convert(date,c.event_date)", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Concepto", "c.concept_code", GetType(Integer), New RepositoryChecked(causal.request_causal_search("row_visible=1 and c.module_code=4 and c.causal_type='E'"), "causal_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Segmento", "c.segment_code", GetType(Integer), New RepositoryChecked(parameter.settings_search_segments("row_visible=1 and s.Id in (1,2)"), "segment_name"), FilterColumnClauseClass.Lookup))
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de funcionario", "c.person_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1 and p.agency_code in (" & sessions.agency_permit & ")"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Usuario Creador", "c.created_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1 and p.agency_code in (" & sessions.agency_permit & ")"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Cantidad", "c.collection_ammount", GetType(Integer), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Valor Recaudado", "c.collection_values", GetType(Integer), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
    End Sub
