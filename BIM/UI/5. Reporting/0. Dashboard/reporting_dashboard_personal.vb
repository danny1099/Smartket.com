...
Public Class reporting_dashboard_personal
    Private reporting As Reporting = Reporting.Instance
    Private criteria_search As String = "month(s.date_sale)=" & Month(Now) & " and year(s.date_sale)=" & Year(Now) & " and s.agency_code in (" & sessions.agency_permit & ")" & " and s." & sessions.query_permit
...
#Region "behaviors"
    ...
    Private Sub reports_show(search_condition As String, criteria_budget As String)
        ...
        With dbs_connections
            ...
            If .Result IsNot Nothing Then
                ...
                Dim source_budgets As DataTable = reporting.reporting_dashboard_person(search_condition, criteria_budget)
                ...
                With source_budgets.Rows(0)
                    ...
                    lbl_object_budget.Text = If(.Item("values_budperson").ToString <> "", FormatNumber(.Item("values_budperson"), 0), "0")
                    percen_value = If(.Item("values_budperson").ToString <> "", (.Item("values_wholesales") / .Item("values_budperson")), 0)
    ...
    Private Sub calendar_changed(sender As Object, e As EventArgs) Handles txt_sales_date.EditValueChanged
        If txt_sales_date.EditValue IsNot Nothing Then
            Dim condition_search As String = "month(s.date_sale)=" & Month(txt_sales_date.EditValue) & " and year(s.date_sale)=" & Year(txt_sales_date.EditValue) & " and s.agency_code in (" & sessions.agency_permit & ")" & " and s." & sessions.query_permit & " and s.segment_code=" & rgo_object_segment.EditValue
            Dim criteria_budget As String = "b.validity_month=" & Month(txt_sales_date.EditValue) & " and b.validity_year=" & Year(txt_sales_date.EditValue) & " and b.row_visible=1" & " and b." & sessions.query_permit

            'ejecuta el proceso de carga de datos
            If criteria_search <> "" Then reports_show(condition_search, criteria_budget)
        End If
    End Sub
