...
Public Class reports_attentions_resumen
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = reporting.reporting_attentions_resumen(criteria_search)
...
#Region "helpers"
    Private Sub objects_condition()
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de creación", "t.created_date", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de finalización", "t.closed_date", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de gestión", "t.topic_code", GetType(Integer), New RepositoryChecked(attention.commercial_attention_topic("row_visible=1"), "topic_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tema de atención", "t.theme_code", GetType(Integer), New RepositoryChecked(attention.commercial_attention_themes("row_visible=1"), "theme_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre del cliente", "t.customer_code", GetType(Integer), New RepositoryChecked(customer.comercial_customer_listed("c.row_visible>=0"), "Nombre del cliente"), FilterColumnClauseClass.Lookup))
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de consultor", "t.person_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1 and p.agency_code in (" & sessions.agency_permit & ")"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de cierre", "t.cause_closure", GetType(Integer), New RepositoryChecked(causal.request_causal_search("row_visible=1 and c.module_code=5 and c.causal_type='S'"), "causal_name"), FilterColumnClauseClass.Lookup))
