...
Public Class reports_returns_resumen
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = If(rgo_object_node.EditValue <> 3, returns.wholesale_returns_showed(criteria_search), returns.wholesale_returns_lookup(criteria_search))
        ...
        If rgo_object_node.EditValue <> 3 Then dgv_object_grid.LevelTree.Nodes.Add("node_table", custom_nodes(AccessibleName))
    End Sub
    ...
    Private Function custom_nodes(table_name As String) As GridView
        If dgv_object_view.Columns.Count > 0 Then
            ...
            Dim notes_source As DataTable = If(rgo_object_node.EditValue = 1, notes_.notes_master_search(table_name, "c.row_affected in (" & String.Join(","c, row_affected) & ")"), returns.wholesale_returns_items("r.Id in (" & String.Join(","c, row_affected) & ")"))
...
#Region "helpers"
    Private Sub objects_condition()
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de factura", "r.date_sale", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de registro", "convert(date,r.event_date)", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de finalización", "r.closed_date", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Orden de compra", "r.purchase_order", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Numero de factura", "r.number_invoice", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Agencia", "r.agency_code", GetType(Integer), New RepositoryChecked(agency.settings_agencys_search("row_visible=1"), "agency_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de funcionario", "r.person_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Usuario Creador", "r.created_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Causal de cierre", "r.causal_closed", GetType(Integer), New RepositoryChecked(causal.request_causal_search("row_visible=1 and c.module_code=6 and c.causal_type='S'"), "causal_name"), FilterColumnClauseClass.Lookup))

        If rgo_object_node.EditValue = 3 Then
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("SKU", "f.sku_item", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Marca", "f.brand_code", GetType(Integer), New RepositoryChecked(brand.services_brands_search("row_visible=1"), "brand_name"), FilterColumnClauseClass.Lookup))
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de producto", "f.product_type", GetType(Integer), New RepositoryChecked(product.services_products_type("row_visible=1"), "product_type"), FilterColumnClauseClass.Lookup))
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de producto", "f.Id", GetType(Integer), New RepositoryChecked(product.services_products_listed("s.row_visible=1"), "product_name"), FilterColumnClauseClass.Lookup))
            txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Estado de producto", "x.col_returns_status", GetType(Integer), New RepositoryChecked(causal.request_causal_search("row_visible=1 and c.module_code=6 and c.causal_type='E'"), "causal_name"), FilterColumnClauseClass.Lookup))
        End If
    End Sub
    ...
    Private Sub node_changed(sender As Object, e As EventArgs) Handles rgo_object_node.EditValueChanged
        If rgo_object_node.EditValue IsNot Nothing Then
            txt_rules_criteria.FilterCriteria = Nothing
            txt_rules_criteria.FilterColumns.Clear()

            'Ejecuta nuevamente el llamado de campos para filtrar
            objects_condition()
        End If
    End Sub
#End Region
