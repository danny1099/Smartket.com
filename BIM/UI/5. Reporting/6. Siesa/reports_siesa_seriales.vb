...
Public Class reports_siesa_seriales
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = reporting.reporting_siesa_seriales(criteria_search)
...
#Region "helpers"
    Private Sub objects_condition()
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Fecha de documento", "convert(date,s.[FECHA_DCTO])", GetType(Date), New RepositoryDate, FilterColumnClauseClass.DateTime))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Numero de serial", "convert(varchar(50),s.[ID_SERIAL])", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de documento", "convert(varchar(50),s.[DOC_INV_TIPO])", GetType(String), New RepositoryChecked(reporting.reporting_siesa_documents("[CODIGO] is not null"), "document_type"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("SKU", "convert(varchar(50),s.[ID_ITEMS])", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Centro de costo", "convert(varchar(50),s.[DOC_INV_CO])", GetType(String), New RepositoryChecked(reporting.reporting_siesa_centers("[CODIGO] is not null"), "center_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Referencia", "convert(varchar(50),s.ID_ITEMS)", GetType(String), New RepositoryChecked(reporting.reporting_siesa_products("[ID_ITEM] is not null"), "product_name"), FilterColumnClauseClass.Lookup))
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Bodega", "convert(varchar(50),s.ID_LOCAL)", GetType(String), New RepositoryChecked(reporting.reporting_siesa_wineries("[ID_LOCAL] is not null"), "winerie_name"), FilterColumnClauseClass.Lookup))
    End Sub
