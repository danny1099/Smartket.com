...
Public Class reports_siesa_facturate
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = reporting.reporting_siesa_facturate(criteria_search)
...
#Region "helpers"
    Private Sub objects_condition()
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Numero de serial", "convert(varchar(50),m.[ID_SERIAL])", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de documento", "convert(varchar(50),s.[ID_TIPDOC])", GetType(String), New RepositoryChecked(reporting.reporting_siesa_documents("[CODIGO] is not null"), "document_type"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("SKU", "convert(varchar(50),s.[ID_ITEM])", GetType(String), New RepositoryItemTextEdit, FilterColumnClauseClass.String))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Centro de costo", "convert(varchar(50),s.[ID_CO])", GetType(String), New RepositoryChecked(reporting.reporting_siesa_centers("[CODIGO] is not null"), "center_name"), FilterColumnClauseClass.Lookup))
    End Sub
