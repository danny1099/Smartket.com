...
Public Class reports_datacredits_resumen
...
#Region "behaviors"
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = reporting.reporting_datacredits_resumen(criteria_search)
...
#Region "helpers"
    Private Sub objects_condition()
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Nombre de solicitante", "c.person_code", GetType(Integer), New RepositoryChecked(person.settings_persons_listed("p.row_visible=1 and p.agency_code in (" & sessions.agency_permit & ")"), "Nombre del funcionario"), FilterColumnClauseClass.Lookup))
        ...
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Cliente", "c.customer_code", GetType(Integer), New RepositoryChecked(customer.comercial_customer_listed("c.row_visible>=0"), "Nombre del cliente"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Canal de ventas", "c.channel_code", GetType(Integer), New RepositoryChecked(channel.settings_channels_search("row_visible=1"), "channel_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Lider de venta", "c.leader_code", GetType(Integer), New RepositoryChecked(person.settings_persons_leaders("d.director_code=3 and l.row_visible=1 and l.agency_code in (" & sessions.agency_permit & ")"), "person_name"), FilterColumnClauseClass.Lookup))
        txt_rules_criteria.FilterColumns.Add(New UnboundFilterColumn("Tipo de datacredito", "c.datacredit_type", GetType(Integer), New RepositoryChecked(causal.request_causal_search("row_visible=1 and c.module_code=3"), "causal_name"), FilterColumnClauseClass.Lookup))
