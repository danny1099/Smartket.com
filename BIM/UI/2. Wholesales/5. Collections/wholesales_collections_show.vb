...
Public Class wholesales_collections_show
    Private criteria_search As String = "month(c.date_collection)=month('" & Now.ToString("yyyy-MM-dd") & "') and year(c.date_collection)=year('" & Now.ToString("yyyy-MM-dd") & "') and c.row_visible=1 and c.agency_code in (" & sessions.agency_permit & ") and c." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = collection.wholesale_collections_show(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Total de recaudos").AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center
                .Columns("Valor Recaudado").AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center
                .Columns("WWID").AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles lbl_object_today.Click, lbl_object_month.Click, lbl_object_lastday.Click
        reports_show(sender.Tag.ToString & " and c.row_visible=1 and c.agency_code in (" & sessions.agency_permit & ") and c." & sessions.query_permit)
    End Sub
    ...
    Private Function total_sales() As String
        ...
        For i As Integer = 0 To dgv_object_view.DataRowCount
            total += CInt(dgv_object_view.GetRowCellValue(i, "Total de recaudos"))
            values += CDbl(dgv_object_view.GetRowCellValue(i, "Valor Recaudado"))
        Next

        'define el total de recaudos
        total_result = total & " (" & FormatCurrency(values, 0) & ")"
...
#Region "options"
    Private Sub created_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New wholesales_collections_create)
    End Sub
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                show_option(New wholesales_collections_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        Else
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        collection.wholesale_collections_erased_option(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
