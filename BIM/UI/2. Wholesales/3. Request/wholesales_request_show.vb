...
Public Class wholesales_request_show
    Private criteria_search As String = "r.row_visible in (3) and r.agency_code in (" & sessions.agency_permit & ") and r." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        Dim source As DataTable = request.wholesale_request_show(criteria_search)
        ...
        If source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("wholesale_code").Visible = False
                .BestFitColumns(True)
                .SelectRow(0)
...
#Region "options"
            ...
            If message_text("Está seguro que desea eliminar los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        request.wholesale_request_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub accepted_option(sender As Object, e As EventArgs) Handles btn_object_accept.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea autorizar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                If (dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Estado")) = "Pendiente" Then
                    Select Case (dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Segmento"))
                        Case "Movil" : show_option(New wholesales_movil_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "wholesale_code"), Me))
                        Case "Hogar" : show_option(New wholesales_hogar_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "wholesale_code"), Me))
                        Case "Libres" : show_option(New wholesales_frees_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "wholesale_code"), Me))
                    End Select
                Else
    ...
    Private Sub rejected_option(sender As Object, e As EventArgs) Handles btn_object_reject.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If (dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Estado")) = "Pendiente" Then
                If message_text("Está seguro que desea rechazar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                    Using new_ As New wholesales_request_reject(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
