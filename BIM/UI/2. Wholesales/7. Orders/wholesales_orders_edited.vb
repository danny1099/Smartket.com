...
Public Class wholesales_orders_edited
...
#Region "constructor"
    Public Sub New(row_selected As Integer)
        ...
        'Define el parametro seleccionado
...
#Region "behaviors"
    ...
    Private Sub search_object()
        Dim table_search As DataTable = orders_.wholesale_orders_search("o.Id=" & row_affected)

        If table_search.Rows.Count = 1 Then
            With table_search.Rows(0)
                cmb_orders_customer.EditValue = CInt(.Item("customer_code"))
                cmb_orders_agency.EditValue = cmb_orders_agency.Properties.GetKeyValueByDisplayText(.Item("agency_name"))
                cmb_orders_deptos.EditValue = cmb_orders_deptos.Properties.GetKeyValueByDisplayText(.Item("department_name"))
                cmb_orders_cities.EditValue = cmb_orders_cities.Properties.GetKeyValueByDisplayText(.Item("city_name"))
                cmb_orders_person.EditValue = CInt(.Item("person_code"))
                cmb_orders_payment.EditValue = cmb_orders_payment.Properties.GetKeyValueByDisplayText(.Item("payment_method"))
                cmb_orders_platform.EditValue = cmb_orders_platform.Properties.GetKeyValueByDisplayText(.Item("dealer_name"))
                txt_orders_address.EditValue = .Item("place_address")
                txt_orders_neighborhood.EditValue = .Item("neighborhood").ToString
                txt_orders_email.EditValue = .Item("email").ToString
                txt_orders_purcharse.EditValue = .Item("purchase_order").ToString
                chk_orders_type.Checked = If(.Item("order_type").ToString = "R", True, False)
            ...
            source_current = trace_to_current(pnl_object_resumen)
        End If
    End Sub
    ...
    Private Sub saved_option(sender As Object, e As EventArgs) Handles btn_object_accept.Click
        If xvp_validate.Validate = True And dgv_object_view.RowCount > 0 Then
            With orders_.sql_command
                ...
                .CommandText = "wholesales_master_orders_edited"
                ...
                .Parameters.Add("@trace_objects", SqlDbType.VarChar, 3000).Value = trace_to_edited(pnl_object_resumen, source_current)
