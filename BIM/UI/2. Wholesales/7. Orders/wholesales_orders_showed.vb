...
Public Class wholesales_orders_showed
    Private criteria_search As String = "month(o.event_date)=month('" & Now.ToString("yyyy-MM-dd") & "') and year(o.event_date)=year('" & Now.ToString("yyyy-MM-dd") & "') and o.row_visible=1  and o.agency_code in (" & sessions.agency_permit & ") and o." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        ...
        source_object = orders_.wholesale_orders_show(criteria_search)
        ...
        If source_object.Rows.Count > 0 Then
            With source_object
                object_label_slopes.Text = String.Format("Pendientes: ({0})", .Select("Estado='Pendiente' or Estado='Documentado'").Count)
                object_label_isolates.Text = String.Format("En Alistamiento: ({0})", .Select("Estado='Alistamiento'").Count)
                object_label_sending.Text = String.Format("En Despacho: ({0})", .Select("Estado='Despachado'").Count)
                object_label_invoice.Text = String.Format("Listo para facturar: ({0})", .Select("Estado='Listo para facturar'").Count)
                object_label_closed.Text = String.Format("Finalizados: ({0})", .Select("Estado='Finalizado'").Count)
                object_label_search.Text = String.Format("Todas los pedidos: ({0})", .Rows.Count)
    ...
    Public Sub reports_show(table_source As DataTable)
        ...
        If table_source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Documentos").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles object_label_slopes.Click, object_label_sending.Click, object_label_search.Click, object_label_isolates.Click, object_label_closed.Click, object_label_invoice.Click
        If source_object.Rows.Count > 0 Then
    ...
    Private Sub selected_rows(sender As Object, e As EventArgs) Handles dgv_object_grid.DoubleClick
        With dgv_object_view
            If .IsRowSelected(.FocusedRowHandle) = True Then
                Using new_ As New wholesales_orders_document(.GetRowCellValue(.FocusedRowHandle, "Id"))
    ...
    Private Function custom_nodes(table_name As String) As GridView
        If dgv_object_view.Columns.Count > 0 Then
            ...
            Dim notes_source As DataTable = If(rgo_object_node.EditValue = 1, notes_.notes_master_search(table_name, "c.row_affected in (" & String.Join(","c, row_affected) & ")"), orders_.wholesale_orders_items("o.Id in (" & String.Join(","c, row_affected) & ")"))
            ...
            If notes_source.Rows.Count > 0 Then
                ...
                With dgv_object_node
                    ...
                    Else
                        ...
                        .Columns("Cantidad").AppearanceHeader.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                    End If
                End With
    ...
    Private Sub selected_date(sender As Object, e As EventArgs) Handles txt_sales_date.EditValueChanged
        If txt_sales_date.EditValue IsNot Nothing Then
            reports_show("month(o.event_date)=month('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and year(o.event_date)=year('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and o.row_visible=1 and o.agency_code in (" & sessions.agency_permit & ") and o." & sessions.query_permit)
        End If
    End Sub
#End Region
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        show_option(New wholesales_orders_created())
    End Sub
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                show_option(New wholesales_orders_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id")))
            End If
        Else
            ...
            If message_text("Está seguro que desea anular los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        orders_.wholesale_orders_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub finally_option(sender As Object, e As EventArgs) Handles btn_object_finally.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea finalizar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_orders_closed(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub noted_option(sender As Object, e As EventArgs) Handles btn_object_notes.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using new_ As New model_object_notes("Wholesales.Masters.Orders", dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"), "Pedidos")
                new_.ShowDialog(start_home)
            End Using
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
    ...
    Private Sub status_option(sender As Object, e As EventArgs) Handles btn_object_status.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea cambiar el estado al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_orders_status(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub product_option(sender As Object, e As EventArgs) Handles btn_object_products.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea ver los productos asociados al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_orders_products(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub documents_option(sender As Object, e As EventArgs) Handles btn_object_document.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea registar el numero de guía al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_orders_document(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
            End If
        Else
    ...
    Private Sub transfer_option(sender As Object, e As EventArgs) Handles btn_object_transfer.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea transferir el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_orders_transfer(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
