...
Public Class wholesales_deliverys_showed
    Private criteria_search As String = "month(d.event_date)=month('" & Now.ToString("yyyy-MM-dd") & "') and year(d.event_date)=year('" & Now.ToString("yyyy-MM-dd") & "') and d.row_visible=1 and d.agency_source in (" & sessions.agency_permit & ") and d." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        ...
        source_object = delivery.wholesale_deliverys_show(criteria_search)
        ...
        If source_object.Rows.Count > 0 Then
            With source_object
                ...
                object_label_accepted.Text = String.Format("Autorizados: ({0})", .Select("Estado='Autorizado'").Count)
                object_label_sending.Text = String.Format("Finalizado: ({0})", .Select("Estado='Finalizado'").Count)
    ...
    Public Sub reports_show(table_source As DataTable)
        ...
        If table_source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("Total a gestionar").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .Columns("Total Gestionado").AppearanceCell.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center
                .BestFitColumns(True)
                .SelectRow(0)
    ...
    Private Sub selected_rows(sender As Object, e As EventArgs) Handles dgv_object_grid.DoubleClick
        With dgv_object_view
            If .IsRowSelected(.FocusedRowHandle) = True Then
                Using new_ As New wholesales_deliverys_products(.GetRowCellValue(.FocusedRowHandle, "Id"))
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles object_label_sending.Click, object_label_search.Click, object_label_accepted.Click, object_label_slopes.Click
        If source_object.Rows.Count > 0 Then
    ...
    Private Function total_values() As String
        ...
        For i As Integer = 0 To dgv_object_view.DataRowCount
            value_total += CDbl(Replace(Replace(Replace(dgv_object_view.GetRowCellValue(i, "Total Items"), ".", ""), "$", ""), "- ", "-"))
        Next

        Return FormatNumber(value_total, 0)
    End Function
    ...
    Private Function custom_nodes(table_name As String) As GridView
        If dgv_object_view.Columns.Count > 0 Then
            ...
            Dim notes_source As DataTable = delivery.wholesale_sending_showed("s.delivery_code in (" & String.Join(","c, row_affected) & ")")
            ...
            If notes_source.Rows.Count > 0 Then
                ...
                With tables_sources
                    ...
                    Dim foreignkey_column As DataColumn = .Tables(1).Columns("delivery_code")
                ...
                With dgv_object_node
                    ...
                    .Columns("delivery_code").VisibleIndex = -1
    ...
    Private Sub node_type(sender As Object, e As EventArgs)
        If dgv_object_view.RowCount > 0 Then
    ...
    Private Sub selected_date(sender As Object, e As EventArgs) Handles txt_sales_date.EditValueChanged
        If txt_sales_date.EditValue IsNot Nothing Then
            reports_show("month(d.event_date)=month('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and year(d.event_date)=year('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and d.row_visible=1 and d.agency_source in (" & sessions.agency_permit & ") and d." & sessions.query_permit)
        End If
    End Sub
#End Region
...
#Region "options"
    Private Sub create_option(sender As Object, e As EventArgs) Handles btn_object_new.Click
        Using new_ As New wholesales_deliverys_create
            new_.ShowDialog(start_home)
        End Using
    ...
    Private Sub edited_option(sender As Object, e As EventArgs) Handles btn_object_edit.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea modificar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_edited(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub finally_option(sender As Object, e As EventArgs) Handles btn_object_finally.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea finalizar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_closed(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
            ...
            If message_text("Está seguro que desea anular los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        delivery.wholesale_deliverys_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub noted_option(sender As Object, e As EventArgs) Handles btn_object_notes.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using new_ As New model_object_notes("Wholesales.Masters.Deliverys", dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"), "Traslados")
                new_.ShowDialog(start_home)
            End Using
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
    ...
    Private Sub status_option(sender As Object, e As EventArgs)
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea cambiar el estado al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_status(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub document_option(sender As Object, e As EventArgs)
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea actualizar el documento de traslado al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_document(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub product_option(sender As Object, e As EventArgs) Handles btn_object_products.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea ver los productos asociados al registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_products(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
    ...
    Private Sub transfer_option(sender As Object, e As EventArgs) Handles btn_object_transfer.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea transferir el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_transfer(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
