...
Public Class wholesales_deliverys_sending
    Private criteria_search As String = "month(s.event_date)=month('" & Now.ToString("yyyy-MM-dd") & "') and year(s.event_date)=year('" & Now.ToString("yyyy-MM-dd") & "') and s.row_visible=1 and s.agency_code in (" & sessions.agency_permit & ") and s." & sessions.query_permit
...
#Region "behaviors"
    ...
    Public Sub reports_show(Optional criteria_search As String = "")
        ...
        source_object = delivery.wholesale_sending_showed(criteria_search)
        ...
        If source_object.Rows.Count > 0 Then
            With source_object
                ...
                quick_search(object_label_accepted, Nothing)
            End With
        End If
    End Sub
    ...
    Public Sub reports_show(table_source As DataTable)
        ...
        If table_source.Rows.Count > 0 Then
            ...
            With dgv_object_view
                ...
                .Columns("delivery_code").Visible = False
    ...
    Private Sub quick_search(sender As Object, e As EventArgs) Handles object_label_sending.Click, object_label_search.Click, object_label_isolates.Click, object_label_accepted.Click
        If source_object.Rows.Count > 0 Then
    ...
    Private Sub selected_rows(sender As Object, e As EventArgs) Handles dgv_object_grid.DoubleClick
        With dgv_object_view
            If .IsRowSelected(.FocusedRowHandle) = True Then
                Using new_ As New wholesales_deliverys_document(.GetRowCellValue(.FocusedRowHandle, "Id"))
    ...
    Private Function total_values() As String
        ...
        For i As Integer = 0 To dgv_object_view.DataRowCount
            value_total += CDbl(Replace(Replace(Replace(dgv_object_view.GetRowCellValue(i, "Valor de traslado"), ".", ""), "$", ""), "- ", "-"))
        Next
    ...
    Private Sub selected_date(sender As Object, e As EventArgs) Handles txt_sales_date.EditValueChanged
        If txt_sales_date.EditValue IsNot Nothing Then
            reports_show("month(s.event_date)=month('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and year(s.event_date)=year('" & Format(txt_sales_date.EditValue, "yyyy-MM-dd") & "') and s.row_visible=1 and s.agency_code in (" & sessions.agency_permit & ") and s." & sessions.query_permit)
        End If
    End Sub
#End Region
...
#Region "options"
    Private Sub finally_option(sender As Object, e As EventArgs) Handles btn_object_finally.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            If message_text("Está seguro que desea finalizar el registro seleccionado?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Using new_ As New wholesales_deliverys_finally(dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"))
                    new_.ShowDialog(start_home)
                End Using
            ...
            If message_text("Está seguro que desea anular los registros seleccionados?", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For i As Integer = 0 To dgv_object_view.DataRowCount - 1
                    If dgv_object_view.IsRowSelected(i) = True Then
                        delivery.wholesale_sending_erase(dgv_object_view.GetRowCellValue(i, "Id"), False)
                    End If
                Next
    ...
    Private Sub noted_option(sender As Object, e As EventArgs) Handles btn_object_notes.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
            Using new_ As New model_object_notes("Wholesales.Masters.Sending", dgv_object_view.GetRowCellValue(dgv_object_view.FocusedRowHandle, "Id"), "Orden Traslado")
                new_.ShowDialog(start_home)
            End Using
        Else
            message_text("La acción no se puede realizar con selección multiple", MessageBoxButtons.OK)
    ...
    Private Sub document_option(sender As Object, e As EventArgs) Handles btn_object_document.Click
        If dgv_object_view.SelectedRowsCount.ToString = 1 Then
